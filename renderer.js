...
function home(){return `<div class="shell">
 <section class="hero" aria-labelledby="home-title"><div class="hero-copy"><p class="hero-kicker">GAN lab / 浙江大学团队</p><h1 id="home-title">AI、人机交互<br>与传统文化<span>。</span></h1><p class="hero-description">研究生成模型与人机协作，探索智能技术<br class="desktop-break">在文化理解、创造与体验中的新可能。</p><div class="hero-actions">${arrow('#/research','了解我们的研究')}<a class="quiet-link" href="#/people">认识团队 ${iconArrow}</a></div></div><figure class="hero-image"><picture><source srcset="assets/culture-computation-concept.webp" type="image/webp"><img src="assets/culture-computation-concept.png" alt="水墨山形与同形采样点云的黑白概念视觉" width="1536" height="1024" fetchpriority="high"></picture></figure><div class="hero-caption"><span>Culture × Computation</span><span>原创概念视觉</span></div></section>
 <section class="research-intro" aria-labelledby="questions-title"><h2 class="section-title" id="questions-title">从生成方法，到人的文化体验。</h2><p class="section-subtitle">研究生成模型本身，也关注人与智能系统怎样共同创作。<br>传统文化为研究带来具体问题，这些主题在不同工作中交汇。</p><div class="perspectives"><div class="perspective-menu" aria-label="研究视角">${perspectives.map((p,i)=>`<a id="perspective-tab-${i}" href="#perspective-panel-${i}" data-perspective="${i}">${p.title}${iconArrow}</a>`).join('')}</div><div class="perspective-panels">${perspectives.map((p,i)=>`<section class="perspective-panel" id="perspective-panel-${i}" aria-labelledby="perspective-tab-${i}">${perspectivePanel(i)}</section>`).join('')}</div></div></section>
...
function pageFor(route) {
  ...
  if (route === '/') {
    ...
    description = 'GAN lab 是浙江大学团队，导师为李泽健。研究生成模型与人机协作，探索智能技术在文化理解、创造与体验中的新可能。';
  } else if (route === '/research') {
    html = research(); title = '研究｜GAN lab'; nav = 'research'; kind = 'CollectionPage';
    description = '研究智能生成的方法，理解人的交互与判断，探索文化创作的新可能。了解 GAN lab 关注的问题与精选研究。';
