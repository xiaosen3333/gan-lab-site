...
浏览器加载预生成正文，只使用 `site-runtime.js` 与 `app.js` 增强交互。首页三研究视角和联系页三交流意图都在初始 HTML 中，无 JavaScript 时全部可读并可通过锚点定位；加载后增强为键盘可操作的 tab。页面导航采用原生链接，支持新标签和复制地址；历史状态保存滚动位置与名录焦点。

旧 `#/research/...`、`#/people/...`、成员查询、联系意图和名录 focus 链接映射到真实路径。已知无详情成员回名录定位，旧管理/预览入口回首页；未知旧 hash 转向不存在的真实路径，由托管服务器返回 404。服务器看不到 URL fragment，客户端兼容跳转不能称为 HTTP 301。
