...
首页选择关系在 `renderer.js` 的 `homepageWorks`；摘要和媒体分别保存在 `research.js`、`projects.js` 原对象中。研究目录独立列出三项研究；所有内容直接可读，不使用 tab。保留旧 `#perspective-panel-0/1/2` 锚点，其中 0 和 2 分别定位 DisBack、Ink Restorer，1 定位首页内容入口。论文页按年份分组并提供真实年份锚点，全部 46 篇同时保留；成员页的项目与作品只从项目署名中的 memberId 派生。联系页只显示邮箱图片和导师主页，不将地址写回正文、alt、JSON-LD 或 mailto。

首页先给简短团队身份；“项目与研究”依次展示 DisBack、MoWorld 与 Ink Restorer；“文化实践与展览”以《运河·生长·万象》为主作品，并列人工智能发展简史图谱与 Artist 1.0 两条展览记录。研究部分通往论文目录，文化部分通往完整项目页，避免重复入口。内容按自然高度呈现，DisBack 不添加装饰图或空白媒体框。首页主视觉与响应式资源保留，首页与项目页的分享图使用 MoWorld，其余页面使用 GAN lab 品牌原图。原始内部材料不进入公开仓库。
...
改动年份折叠时，不只检查最终开关状态。使用有传统滚动条的浏览器，在能容纳全部折叠年份的高视口（如1352×1400）连续展开、收起至少三轮，比较页头、年份标题的横坐标，确认滚动条出现或消失不造成整页横移。再检查普通高度下页面中段和底部年份、键盘Enter/Space和默认2026展开。

测量点击前后位置时，用当前可见标题的实际点击坐标；自动化定位器可能在点击前把元素滚到视口中央，不能将其引起的位移误判为页面缺陷。记录改前复现与改后同路径结果，不能用静态HTML检查替代动态布局验证。
