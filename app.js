...
function contact(intent='academic'){if(!contactIntents[intent])intent='academic';const c=contactIntents[intent];return `<div class="shell">${heading('交流与合作','不同的背景，可以从共同关心的问题开始。','交流与合作')}<section class="contact-channel" aria-label="联系导师"><span class="label">导师公开学术邮箱</span><div class="contact-email"><img src="assets/contact-channel.png" alt="李泽健老师的学术邮箱，地址见图片" width="732" height="89"></div><small>李泽健 · 浙江大学</small><a class="inline-link" href="${mentorHome}" target="_blank" rel="noopener">查看导师主页与参与说明 ${iconArrow}</a></section><div class="contact-layout"><div class="contact-tabs" role="tablist" aria-label="交流意图">${Object.entries(contactIntents).map(([key,item])=>`<button id="contact-tab-${key}" role="tab" aria-selected="${key===intent}" aria-controls="contact-panel" tabindex="${key===intent?0:-1}" data-intent="${key}">${item.label}</button>`).join('')}</div><section class="contact-panel" id="contact-panel" role="tabpanel" aria-labelledby="contact-tab-${intent}"><h2>${c.title}</h2><p>${c.body}</p><h3>下一步如何展开</h3><p>${c.next}</p><h3>交流时可以带上</h3><ul>${c.prepare.map(item=>`<li>${item}</li>`).join('')}</ul>${inline(intent==='join'?'#/people':'#/research',intent==='join'?'先了解团队':'先阅读相关研究')}</section></div></div>`;}
