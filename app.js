...
function memberLinks(ids){return ids.map(id=>{const m=members.find(m=>m.id===id);return `<a href="#/people/${m.id}">${m.name}</a>`;}).join('');}
...
function personCard(m){return `<article class="person-card" id="member-${m.id}" tabindex="-1"><a class="person-card-link" href="#/people/${m.id}"><div class="person-card-heading"><h3>${m.name}</h3><span class="person-title">${m.title||'团队成员'}</span></div>${presentation.memberEnglish&&m.english?`<p class="person-english">${m.english}</p>`:''}<span class="person-navigation" aria-hidden="true">${iconArrow}</span></a></article>`;}
...
function contact(intent='academic'){if(!contactIntents[intent])intent='academic';const c=contactIntents[intent];return `<div class="shell">${heading('交流与合作','不同的背景，可以从共同关心的问题开始。','交流与合作')}<div class="contact-layout"><div class="contact-tabs" role="tablist" aria-label="交流意图">${Object.entries(contactIntents).map(([key,item])=>`<button id="contact-tab-${key}" role="tab" aria-selected="${key===intent}" aria-controls="contact-panel" tabindex="${key===intent?0:-1}" data-intent="${key}">${item.label}</button>`).join('')}</div><section class="contact-panel" id="contact-panel" role="tabpanel" aria-labelledby="contact-tab-${intent}"><h2>${c.title}</h2><p>${c.body}</p><h3>下一步如何展开</h3><p>${c.next}</p><h3>交流时可以带上</h3><ul>${c.prepare.map(item=>`<li>${item}</li>`).join('')}</ul><div class="contact-channel"><span class="label">导师公开学术邮箱</span><a class="contact-email" href="mailto:zejianlee@zju.edu.cn">zejianlee@zju.edu.cn ${iconArrow}</a><small>李泽健 · 浙江大学<br>点击邮箱使用你的邮件应用联系。</small><a class="inline-link" href="${mentorHome}" target="_blank" rel="noopener">查看导师主页与参与说明 ${iconArrow}</a></div>${inline(intent==='join'?'#/people':'#/research',intent==='join'?'先了解团队':'先阅读相关研究')}</section></div></div>`;}
function getRoute(){const raw=location.hash.slice(1)||'/';const [path,query]=raw.split('?');if(path==='/admin'||path.startsWith('/admin/')||path==='/preview'){history.replaceState(null,'','#/');return {path:'/',params:new URLSearchParams()};}const params=new URLSearchParams(query);if((path==='/outputs'||path==='/people')&&members.some(m=>m.id===params.get('member'))){const memberPath='/people/'+params.get('member');history.replaceState(null,'','#'+memberPath);return {path:memberPath,params:new URLSearchParams()};}return {path,params};}
function render({focus=true}={}){const {path,params}=getRoute();let html,title,nav='';
...
window.addEventListener('hashchange',()=>render());render({focus:false});
